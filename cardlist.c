...
void printCardList (CARDLIST* cardlist)
{
	...
	NODE *current = cardlist->first;
	printf ("[0: \"%s\" (V: %d, C: %d, A: %d)", current->card->name, current->card->life, current->card->cost,
            current->card->attack);
	int i = 1;
	while (current->next != NULL)
	{
		printf (",\n%i: \"%s\" (V: %d, C: %d, A: %d)", i, current->next->card->name, current->next->card->life,
                current->next->card->cost, current->next->card->attack);
		current = current->next;
		i++;
	}
	printf ("]\n");
